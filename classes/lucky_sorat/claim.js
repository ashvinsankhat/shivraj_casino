...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.lucky_sorat:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Lucky sorat please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await LuckySoratPlayings.findOne(ra_wh, ra_project).lean();
		...
		respSendActions.SendDataToUser(client, "LUCKY_SORAT_CLAIM", gameTrackInfo);
		redisClass.del(rcKey);
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "LUCKY_SORAT_ALL_CLAIM", response);
		redisClass.del(rcKey);
