module.exports.addWallet = async(id, chips, tType, t, tbInfo) => {
    try {
        ...
        setInfo['$inc']['chips'] = Number(chips.toFixed(2));

        csl("\naddWallet setInfo :: ", setInfo);
        var tranferAmount = Number(chips.toFixed(2));
        
        var fChips = Number(Number(userInfo.chips).toFixed(2)) + Number(chips.toFixed(2));
        if (Number(fChips) < 0) {
            fChips = 0;
        }
        csl("addWallet userInfo :: ", userInfo);
        ...
        if (typeof tType != 'undefined') {
            ...
            let walletTrack = {
                ...
                bet_value           : (tbInfo && tbInfo.bet_value) ? tbInfo.bet_value : 0,
        ...
        if(Object.keys(setInfo["$inc"]).length > 0){
            for(let key in setInfo["$inc"]){
                setInfo["$inc"][key] = parseFloat(setInfo["$inc"][key].toFixed(2));
            }
        }
        if(Object.keys(setInfo["$inc"]).length == 0){
            delete setInfo["$inc"];
