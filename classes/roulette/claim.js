...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.roulette:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Roulette please wait some time....!", rcRadis);
			return false;
		}
		...
		if (typeof requestData.g_id != "undefined" && requestData.g_id != "") {
			wh = {
		...
		let gameInfo = await RoulettePlayings.findOne(ra_wh, ra_project).lean();
		...
		respSendActions.SendDataToUser(client, "ROULETTE_CLAIM", gameTrackInfo);
		redisClass.del(rcKey);
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "ROULETTE_ALL_CLAIM", response);
		redisClass.del(rcKey);
