...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.lucky_cards_12:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Lucky cards 12 please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await LuckyCard12Playings.findOne(ra_wh, ra_project).lean();
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "LUCKY_CARD_12_ALL_CLAIM", response);
		redisClass.del(rcKey);
