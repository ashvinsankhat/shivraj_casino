...
module.exports.place_bet = async (requestData, client) => {
	try {
		...
		let gameInfos = await LuckyCardPlayings.findOne({}, {}).lean();
		...
		csl("\nplace_bet  bet_value : ", uChips, bet_value);
		...
		let gameInfo = await LuckyCardPlayings.findOneAndUpdate({}, updateData, {
			new: true,
		});
		...
		bet_info["game_type"] = "lucky_cards_16";
...
module.exports.repeat_bet = async (requestData, client) => {
	try {
		...
		let card_details = requestData.card_details;
		csl("\nrepeat_bet card_details : ", card_details);
		let incease_card_bet = {};
		for (let keys in card_details) {
			incease_card_bet["total_bet_on_cards." + keys] = Number(
				card_details[keys]
			);
			csl("repeat_bet card_details : ", card_details[keys]);
			bet_value = bet_value + Number(card_details[keys]);
		}

		csl("\nrepeat_bet bet_value : ", uChips, bet_value);
		...
		updateData["$push"]["bet_users_cards." + client._id] = ticketId;
		csl("repeat_bet updateData : ", updateData);
