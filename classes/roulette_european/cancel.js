...
module.exports.cancel = async (requestData, client) => {
	try {
		...
		let gameInfo = await RouletteEuropeanPlayings.findOne({}, {}).lean();
		...
		let gameInfos = await RouletteEuropeanPlayings.findOneAndUpdate(
			{},
			updateData,
			{ new: true }
		);
		...
		let room = "Admin_" + gameInfos._id.toString();
