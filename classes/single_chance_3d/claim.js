...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.singple_change_3d:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Single change 3d please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await SingleChance3dPlayings.findOne(
			ra_wh,
			ra_project
		).lean();
