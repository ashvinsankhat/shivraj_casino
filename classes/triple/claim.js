...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.triple:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Triple please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await TriplePlayings.findOne(ra_wh, ra_project).lean();
		...
		respSendActions.SendDataToUser(client, "TRIPLE_CLAIM", gameTrackInfo);
		redisClass.del(rcKey);
