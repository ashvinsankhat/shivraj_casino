...
module.exports.place_bet = async (requestData, client) => {
	try {
		...
		let gameInfos = await TriplePlayings.findOne({}, {}).lean();
		...
		for (let keys in card_details) {
			...
			if (keys.length == 1) {
				final_number = "0-0-" + keys;
			} else if (keys.length == 3) {
				final_number = "0-" + keys;
			}

			let value = incease_card_bet["total_bet_on_cards." + final_number]
				? incease_card_bet["total_bet_on_cards." + final_number]
				: 0;
			incease_card_bet["total_bet_on_cards." + final_number] =
				value + Number(card_details[keys]);

			incease_card_bet["user_bets." + keys] = Number(card_details[keys]);
		...
		csl("\nplace_bet net_bet_value : ", net_bet_value);
		...
		let gameInfo = await TriplePlayings.findOneAndUpdate({}, updateData, {
			new: true,
		});
		...
		bet_info["game_type"] = "triple_chance";
		...
		respSendActions.SendDataToDirect(client.sck_id, "TRIPLE_PLACE_BET", betIn);
		delete client.place_bet;
...
module.exports.repeat_bet = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToDirect(client.sck_id, "TRIPLE_REPEAT_BET", betIn);
		delete client.repeat_bet;
		return true;
	} catch (e) {
