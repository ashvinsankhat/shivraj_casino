...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.singple_change:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Single change please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await SingleChancePlayings.findOne(ra_wh, ra_project).lean();
		csl("claim gameInfo : ", gameInfo);
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "SINGLE_CHANCE_ALL_CLAIM", response);
		redisClass.del(rcKey);
