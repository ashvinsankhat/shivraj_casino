...
module.exports.place_bet = async (requestData, client) => {
	try {
		...
		let gameInfos = await RouletteZero3dPlayings.findOne({}, {}).lean();
		...
		let gameInfo = await RouletteZero3dPlayings.findOneAndUpdate(
			{},
			updateData,
			{ new: true }
		);
		...
		bet_info["game_type"] = "roulette_zero_3d";
