module.exports.get_game_info = async(requestData, client) => {
    try{
        if(typeof client._id == "undefined"){
            respSendActions.SendDataToDirect(client.sck_id, 'ANDAR_BAHAR_GAME_INFO', requestData , true, "1000" , "User session not set, please restart game!", "Error!");
            return true;
        }
        ...
        let gameInfo = await AndarBahars.findOne(wh, project).lean();
        ...
        let gameBetInfo = await Lucky16CardTracks.findOne(wh1,project1).sort({_id:-1}).lean()
        ...
        csl("check_winner gameInfo.last_win_cards : ", gameInfo.last_win_cards);
        ...
        if(typeof gameInfo.win_info != "undefined" && typeof gameInfo.win_info.last_card_throw != "undefined"){
            
            let center_card = gameInfo.win_info.cards;
            
            let bahar_cards = "";
            let under_cards = "";

            if( gameInfo.win_info.last_card_throw == "Bahar" ){
                bahar_cards = gameInfo.win_info.deal_card[gameInfo.win_info.deal_card.length - 1]
                under_cards = gameInfo.win_info.deal_card[gameInfo.win_info.deal_card.length - 2]
            }
            if( gameInfo.win_info.last_card_throw == "Under" ){
                under_cards = gameInfo.win_info.deal_card[gameInfo.win_info.deal_card.length - 1]
                bahar_cards = gameInfo.win_info.deal_card[gameInfo.win_info.deal_card.length - 2]
            }

            gameInfo.win_info = {
                center_card : center_card,
                bahar_cards : bahar_cards,
        ...
        respSendActions.SendDataToUser(client, 'ANDAR_BAHAR_GAME_INFO', gameInfo);
...
module.exports.close_game = async(requestData, client) => {
    try{
        ...
        respSendActions.SendDataToUser(client, 'ANDAR_BAHAR_CLOSE_GAME', {
            done : true
        });
...
module.exports.auto_on_off = async(requestData, client) => {
    try{
        if(typeof client._id == "undefined"){
            respSendActions.SendDataToDirect(client.sck_id, 'ANDAR_BAHAR_PLACE_BET', requestData , true, "1000" , "User session not set, please restart game!", "Error!");
            return true;
        }
        ...
        respSendActions.SendDataToUser(client, 'ANDAR_BAHAR_AUTO_ON_OFF', {
...
module.exports.get_specific_game_info = async(requestData, client) => {
    try{
        if(typeof client._id == "undefined"){
            respSendActions.SendDataToDirect(client.sck_id, 'ANDAR_BAHAR_GAME_SPECIFIC_INFO', requestData , true, "1000" , "User session not set, please restart game!", "Error!");
            return true;
        }
        let wh = {
        ...
        respSendActions.SendDataToUser(client, 'ANDAR_BAHAR_GAME_SPECIFIC_INFO', gameBetInfo);
        return false
    }catch(e) {
