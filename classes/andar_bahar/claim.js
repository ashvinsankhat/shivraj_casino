...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.andar_bahar:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Andar Bahar please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await AndarBahars.findOne(ra_wh, ra_project).lean();
		...
		// let pay_amount = gameTrackInfo.total_win_amount - gameTrackInfo.commission_amount;
		// csl("claim pay_amount : ", pay_amount);

		await addWallet(
			gameTrackInfo.user_id,
			gameTrackInfo.total_win_amount,
			2,
			"Andar Bahar One Claim",
			gameTrackInfo
		);
		...
		respSendActions.SendDataToUser(client, "ANDAR_BAHAR_CLAIM", gameTrackInfo);
		redisClass.del(rcKey);
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "ANDAR_BAHAR_ALL_CLAIM", response);
		redisClass.del(rcKey);
