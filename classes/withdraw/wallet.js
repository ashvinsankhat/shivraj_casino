module.exports.deductWallet = async(id, chips, tType, t, tbInfo) => {
    try {
        ...
        if(( userInfo.game_winning) >= (-1 * chips)){
            setInfo['$inc']['game_winning'] = Number(Number(chips).toFixed(2));
        }
        csl("\ndedudctWallet setInfo :: ", setInfo);

        var tranferAmount = chips;
        chips = tranferAmount;
        var fChips = Number((Number(chips) + Number(userInfo.chips)).toFixed(2));
        if (Number(fChips) < 0) {
            fChips = 0;w
        }
        csl("dedudctWallet userInfo :: ", userInfo);
...
module.exports.addWallet = async(id, chips, tType, t, tbInfo) => {
    try {
        ...
        setInfo['$inc']['game_winning'] = Number(Number(chips).toFixed(2));
        userInfo.game_winning = Number(userInfo.game_winning) + Number(chips);
