...
module.exports.claim = async (requestData, client) => {
	try {
		...
		let rcKey = "Claim.roulette_zero:" + client._id.toString();
		let rcRadis = await redisClass.setnx(rcKey, 1, 30);
		if (rcRadis == 0) {
			csl("Roulette zero please wait some time....!", rcRadis);
			return false;
		}
		...
		let gameInfo = await RouletteZeroPlayings.findOne(ra_wh, ra_project).lean();
...
module.exports.all_claim = async (requestData, client) => {
	try {
		...
		respSendActions.SendDataToUser(client, "ROULETTE_ZERO_ALL_CLAIM", response);
		redisClass.del(rcKey);
