...
module.exports.place_bet = async (requestData, client) => {
	try {
		...
		let gameInfos = await RouletteZeroPlayings.findOne({}, {}).lean();
		...
		for (let keys in card_details) {
			if (keys == "1st_12" || keys == "2nd_12" || keys == "3rf_12") {
				...
				if (specific_number.indexOf("00") != -1) {
					respSendActions.SendDataToDirect(
						client.sck_id,
						"ROULETTE_ZERO_PLACE_BET",
						requestData,
		...
		let gameInfo = await RouletteZeroPlayings.findOneAndUpdate({}, updateData, {
			new: true,
		});
		...
		bet_info["game_type"] = "roulette_zero";
...
module.exports.repeat_bet = async (requestData, client) => {
	try {
		...
		let project = {};
		let lastBetInfo = await Lucky16CardTracks.findOne(wh, project).sort({
			_id: -1,
		});
