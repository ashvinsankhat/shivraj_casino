...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await RouletteEuropeanPlayings.findOne(wh1, {}).lean();
	...
	scl("roulette_european_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 5) {
		...
		gameInfo = await RouletteEuropeanPlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
	...
	respSendActions.FireEventToTable(
		...
		"ROULETTE_EUROPEAN_WINNER_DECLARE",
		response
	);
				...
				for (let key = 0; key < ticket_ids.length; key++) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await RouletteEuropeanPlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
