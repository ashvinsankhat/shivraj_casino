...
module.exports.roulette_european_start_game = async() => {

    let wh = {
    }
    let project = {
    }
    let gameInfo = await RouletteEuropeanPlayings.findOne(wh, project).lean();
    csl("roulette_european_start_game userInfos : ", gameInfo);
    if(gameInfo == null){
        let insertData = {
        ...
        gameInfo = await RouletteEuropeanPlayings.create(insertData);
    }
    csl("roulette_european_start_game userInfos : ", gameInfo);
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        if(gameInfo.last_win_cards.length > 10){
            ...
            csl("roulette_european_start_game lastCards : ", lastCards);
            
            updateData["$set"] = {
                last_win_cards : lastCards
            }
        }
        csl("roulette_european_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await RouletteEuropeanPlayings.findOneAndUpdate(wh, updateData, {new :true});
        csl("roulette_european_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "ROULETTE_EUROPEAN_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("roulette_european_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("roulette_european_start_game  delayRes: ", delayRes);
        ...
        console.log("roulette_european_start_game Game state is different......!");
    }
    ...
    for(let user_id_keys in bet_users_ticktes){
        ...
        if(ticket_ids.length != 0){
            ...
            if(userInfo != null){
                ...
                for(let key = 0 ; key < ticket_ids.length ; key++){
                    ...
                    if(gameBetInfo != null){
                        ...
                        if(total_win_amount > 0){
                            ...
                            let gameInfo1 = await RouletteEuropeanPlayings.findOneAndUpdate(wh, updateData, {new :true});
                ...
                let en = "ROULETTE_EUROPEAN_WIN";
                if(total_win == 0){
                    en = "ROULETTE_EUROPEAN_LOSE";
                }else{
                    response.take = true;
                }
