...
module.exports.triple_start_game = async () => {
	let wh = {};
	let project = {};
	let gameInfo = await TriplePlayings.findOne(wh, project).lean();
	csl("triple_start_game userInfos : ", gameInfo);
	...
	if (gameInfo == null) {
		...
		gameInfo = await TriplePlayings.create(insertData);
	}
	csl("triple_start_game userInfos : ", gameInfo);
	...
	if (
		...
		csl("triple_start_game wh , project 1: ", wh, updateData);

		gameInfo = await TriplePlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		});
		csl("triple_start_game userInfos : ", gameInfo);
		...
		csl("triple_start_game : ", job_id, new Date(), new Date(turnExpireTime));
		...
		csl("triple_start_game  delayRes: ", delayRes);
		...
		console.log("triple_start_game Game state is different......!");
	}
...
module.exports.game_winning = async gameInfo => {
	...
	let win_array = [];
	let numbers = gameInfo.win_card.split("-");

	let fnum = numbers[0];
	win_array.push(fnum);

	let snum = numbers[0] + "-" + numbers[1];
	win_array.push(snum);
				...
				if (gameInfo.winner_users.indexOf(ticket_ids[key]) == -1) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await TriplePlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
