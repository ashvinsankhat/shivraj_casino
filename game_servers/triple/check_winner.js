...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await TriplePlayings.findOne(wh1, {}).lean();
	...
	scl("triple_chance_check_winner userInfos : ", gameInfo);

	if (gameInfo.last_win_cards.length > 6) {
				...
				if (gameBetInfo != null) {
					...
					for (let keys in card_details) {
						...
						if (win_array.indexOf(keys) != -1) {
							...
							let bet_count = bet_value;

							if (keys.length == 1) {
								win_amount = Number(bet_count) * 9;
							} else if (keys.length == 3) {
								win_amount = Number(bet_count) * 90;
							} else if (keys.length == 5) {
								win_amount = Number(bet_count) * 900;
							}
