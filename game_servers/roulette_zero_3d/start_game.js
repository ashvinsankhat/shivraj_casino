...
module.exports.roulette_zero_3d_start_game = async() => {
    ...
    csl("roulette_zero_3d_start_game userInfos : ", gameInfo);
    if(gameInfo == null){
        let insertData = {
        ...
        gameInfo = await RouletteZero3dPlayings.create(insertData);
    }
    csl("roulette_zero_3d_start_game userInfos : ", gameInfo);
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        if(gameInfo.last_win_cards.length > 10){
            ...
            csl("roulette_zero_3d_start_game lastCards : ", lastCards);
            
            updateData["$set"] = {
                last_win_cards : lastCards
            }
        }
        csl("roulette_zero_3d_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await RouletteZero3dPlayings.findOneAndUpdate(wh, updateData, {new :true});
        csl("roulette_zero_3d_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "ROULETTE_ZERO_3D_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("roulette_zero_3d_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("roulette_zero_3d_start_game  delayRes: ", delayRes);
        ...
        console.log("roulette_zero_3d_start_game Game state is different......!");
    }
    ...
    for(let user_id_keys in bet_users_ticktes){
        ...
        if(ticket_ids.length != 0){
            ...
            if(userInfo != null){
                ...
                for(let key = 0 ; key < ticket_ids.length ; key++){
                    ...
                    if(gameBetInfo != null){
                        ...
                        if(total_win_amount > 0){
                            ...
                            let gameInfo1 = await RouletteZero3dPlayings.findOneAndUpdate(wh, updateData, {new :true});
                ...
                let en = "ROULETTE_ZERO_3D_WIN";
                if(total_win == 0){
                    en = "ROULETTE_ZERO_3D_LOSE";
                }else{
                    response.take = true;
                }
