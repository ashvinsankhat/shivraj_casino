...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await RouletteZero3dPlayings.findOne(wh1, {}).lean();
	...
	let gameInfo = await RouletteZero3dPlayings.findOneAndUpdate(wh, updateData, {
		new: true,
	}).lean();
	scl("roulette_zero_3d_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 10) {
		...
		gameInfo = await RouletteZero3dPlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
	...
	respSendActions.FireEventToTable(
		...
		"ROULETTE_ZERO_3D_WINNER_DECLARE",
		response
	);
					...
					if (gameBetInfo != null) {
						...
						for (let keys in card_details) {
							...
							if (bet_numbers.indexOf(win_array) != -1) {
								...
								win_amount = win_amount / Number(bet_numbers.length);
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await RouletteZero3dPlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
