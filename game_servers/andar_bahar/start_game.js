...
module.exports.andar_bahar_start_game = async() => {
    ...
    csl("andar_bahar_start_game userInfos : ", gameInfo);
    ...
    if(gameInfo == null){
        ...
        gameInfo = await AndarBahars.create(insertData);
    }
    csl("andar_bahar_start_game userInfos : ", gameInfo);
    ...
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        csl("andar_bahar_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await AndarBahars.findOneAndUpdate(wh, updateData, {new :true});
        csl("andar_bahar_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "ANDAR_BAHAR_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("andar_bahar_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("andar_bahar_start_game  delayRes: ", delayRes);
        ...
        console.log("andar_bahar_start_game Game state is different......!");
    }
