const { start_game_timer } = require("./start_gst");
...
module.exports.single_chance_start_game = async () => {
	let wh = {};
	let project = {};
	let gameInfo = await SingleChancePlayings.findOne(wh, project).lean();
	csl("single_chance_start_game userInfos : ", gameInfo);
	...
	if (gameInfo == null) {
		...
		gameInfo = await SingleChancePlayings.create(insertData);
	}
	csl("single_chance_start_game userInfos : ", gameInfo);
	...
	if (
		...
		csl("single_chance_start_game wh , project 1: ", wh, updateData);

		gameInfo = await SingleChancePlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		});
		csl("single_chance_start_game userInfos : ", gameInfo);
		...
		csl("single_chance_start_game  delayRes: ", delayRes);
		...
		console.log("single_chance_start_game Game state is different......!");
	}
...
module.exports.game_winning = async gameInfo => {
	...
	for (let user_id_keys in bet_users_ticktes) {
		...
		if (ticket_ids.length != 0) {
				...
				if (gameInfo.winner_users.indexOf(ticket_ids[key]) == -1) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await SingleChancePlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
                            ...
                            */
							...
							if (auto_claim) {
								await claim_tracks(total_win_amount, {
									_id: gameBetInfo.user_id,
								});
								await addWallet(
									gameBetInfo.user_id,
                            ...
                            */
							...
							await loss_tracks(total_bet_amount, 0, commission_rate, {
								_id: gameBetInfo.user_id,
							});
			...
			let en = "SINGLE_CHANCE_WIN";
			if (total_win == 0) {
				en = "SINGLE_CHANCE_LOSE";
			}
