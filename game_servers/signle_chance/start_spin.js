...
module.exports.start_spin = async table_id => {
	try {
		...
		let gameInfos = await SingleChancePlayings.findOne(wh, {}).lean();
		...
		scl("single_chance_start_spin userInfos : ", gameInfo);
...
module.exports.get_percentage_based_cards = async (gameInfo, gameConfig) => {
	...
	if (in_percentage_card.length > 0) {
		csl("get_best_card  in_percentage_card: ", in_percentage_card);
