...
module.exports.lucky_16_start_game = async() => {
    ...
    csl("lucky_card_16_start_game userInfos : ", gameInfo);
    if(gameInfo == null){
        let insertData = {
        ...
        gameInfo = await LuckyCardPlayings.create(insertData);
    }
    csl("lucky_16_start_game userInfos : ", gameInfo);
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        if(gameInfo.last_win_cards.length > 10){
            ...
            csl("lucky_16_start_game lastCards : ", lastCards);
            
            updateData["$set"] = {
                last_win_cards : lastCards
            }
        }
        csl("lucky_16_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await LuckyCardPlayings.findOneAndUpdate(wh, updateData, {new :true});
        csl("lucky_16_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "LUCKY_CARD_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("lucky_16_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("lucky_16_start_game  delayRes: ", delayRes);
        ...
        console.log("lucky_16_start_game Game state is different......!");
    }
