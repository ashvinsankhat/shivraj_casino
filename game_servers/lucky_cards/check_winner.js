...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await LuckyCardPlayings.findOne(wh1, {}).lean();
	...
	scl("lucky_cards_16_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 10) {
		...
		gameInfo = await LuckyCardPlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
				...
				for (let key = 0; key < ticket_ids.length; key++) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await LuckyCardPlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
