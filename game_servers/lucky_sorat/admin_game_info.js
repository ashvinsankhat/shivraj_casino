module.exports.get_admin_game_info = async gameInfo => {
	try {
		...
		let init_timer =
			typeof gameDataInfo.game_config != "undefined" &&
			typeof gameDataInfo.game_config.NEXT_GAME_TIME != "undefined"
				? gameDataInfo.game_config.NEXT_GAME_TIME
				: 5;
		csl("get_admin_game_info init_timer : ", init_timer);

		let game_timer =
			typeof gameDataInfo.game_config != "undefined" &&
			typeof gameDataInfo.game_config.DRAW_TIME != "undefined"
				? gameDataInfo.game_config.DRAW_TIME
				: 60;
		csl("get_admin_game_info game_timer : ", game_timer);

		let spin_timer =
			typeof gameDataInfo.game_config != "undefined" &&
			typeof gameDataInfo.game_config.SPIN_TIME != "undefined"
				? gameDataInfo.game_config.SPIN_TIME
				: 5;
		csl("get_admin_game_info spin_timer : ", spin_timer);

		let timer = 0;
		if (
			gameDataInfo.game_state == "init_game" ||
			gameDataInfo.game_state == "finish_state"
		) {
			timer =
				init_timer -
				(await commonClass.getTimeDifference(
					new Date(gameDataInfo.game_time.init_time),
					new Date(),
					"second"
				));
			...
			timer =
				game_timer -
				(await commonClass.getTimeDifference(
					new Date(gameDataInfo.game_time.gst_time),
					new Date(),
					"second"
				));
			...
			timer =
				spin_timer -
				(await commonClass.getTimeDifference(
					new Date(gameDataInfo.game_time.spin_time),
					new Date(),
					"second"
				));
		}
		...
		let bet_value = 0;
		if (
			gameDataInfo.game_state == "game_timer_start" ||
			gameDataInfo.game_state == "start_spin"
		) {
			let wh1 = {
			...
			let project1 = {};
			let lastBetInfo = await Lucky16CardTracks.findOne(wh1, project1).sort({
				_id: -1,
			});
