...
module.exports.start_spin = async table_id => {
	try {
		...
		let gameInfos = await LuckySoratPlayings.findOne(wh, {}).lean();
		...
		scl("lucky_sorat_start_spin userInfos : ", gameInfo);
...
module.exports.get_selected_cards = async (gameInfo, gameConfig) => {
	csl("get_selected_cards  gameInfo: ", gameInfo);

	let x_cards_deatils = await GameConfigs.findOne({
		game_type: "lucky_sorat",
	}).lean();
	csl("get_selected_cards  x_cards_deatils: ", x_cards_deatils);
