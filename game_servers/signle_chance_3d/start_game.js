...
module.exports.single_chance_3d_start_game = async() => {

    let wh = {
    }
    let project = {
    }
    let gameInfo = await SingleChance3dPlayings.findOne(wh, project).lean();
    csl("single_chance_3d_start_game userInfos : ", gameInfo);
    ...
    if(gameInfo == null){
        ...
        gameInfo = await SingleChance3dPlayings.create(insertData);
    }
    csl("single_chance_3d_start_game userInfos : ", gameInfo);
    ...
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        if(gameInfo.last_win_cards.length > 10){
            ...
            csl("start_game lastCards : ", lastCards);
            
            updateData["$set"] = {
                last_win_cards : lastCards
            }
        }
        csl("single_chance_3d_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await SingleChance3dPlayings.findOneAndUpdate(wh, updateData, {new :true});
        csl("single_chance_3d_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "SINGLE_CHANCE_3D_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("single_chance_3d_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("single_chance_3d_start_game  delayRes: ", delayRes);
        ...
        console.log("single_chance_3d_start_game Game state is different......!");
    }
...
module.exports.game_winning = async(gameInfo) => {
    ...
    let win_array = [gameInfo.win_card];
    ...
    let n_reward = (typeof gameInfo.game_config != "undefined" && typeof gameInfo.game_config.REWARD_NORMAL != "undefined")?gameInfo.game_config.REWARD_NORMAL : 9;
    ...
    for(let user_id_keys in bet_users_ticktes){
        ...
        if(ticket_ids.length != 0){
                ...
                if(gameInfo.winner_users.indexOf(ticket_ids[key]) == -1){
                    ...
                    if(gameBetInfo != null){
                        ...
                        if(total_win_amount > 0){
                            ...
                            let gameInfo1 = await LuckyCardPlayings.findOneAndUpdate(wh, updateData, {new :true});
            ...
            let en = "SINGLE_CHANCE_3D_WIN";
            if(total_win == 0){
                en = "SINGLE_CHANCE_3D_LOSE";
            }
