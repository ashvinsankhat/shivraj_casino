...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await SingleChance3dPlayings.findOne(wh1, {}).lean();
	...
	let gameInfo = await SingleChance3dPlayings.findOneAndUpdate(wh, updateData, {
		new: true,
	}).lean();
	scl("single_chance_3d_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 10) {
		...
		gameInfo = await SingleChance3dPlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
					...
					if (gameBetInfo != null) {
						...
						for (let keys in card_details) {
							...
							if (win_array.indexOf(keys) != -1) {
								...
								win_amount = Number((Number(bet_value) * n_reward) / 5);
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await SingleChance3dPlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
