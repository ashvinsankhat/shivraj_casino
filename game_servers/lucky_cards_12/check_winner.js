...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await LuckyCard12Playings.findOne(wh1, {}).lean();
	...
	let gameInfo = await LuckyCard12Playings.findOneAndUpdate(wh, updateData, {
		new: true,
	}).lean();
	scl("lucky_cards_12_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 10) {
		...
		gameInfo = await LuckyCard12Playings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
...
module.exports.add_winning_amount = async gameInfo => {
	...
	csl("add_winning_amount  gameConfig: ", gameConfig);
	...
	if (
		...
		retailers = Object.keys(gameConfig.win_loss_user.user_id);
		csl("add_winning_amount  retailers: ", retailers);
	}
	...
	for (let user_id_keys in bet_users_ticktes) {
		...
		if (ticket_ids.length != 0) {
			...
			if (userInfo != null) {
				...
				for (let key = 0; key < ticket_ids.length; key++) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await LuckyCard12Playings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
                            ...
                            */
							...
							if (retailers.indexOf(gameBetInfo.user_id.toString()) != -1) {
								let updateData = {
									$unset: {},
								};
								updateData["$unset"][
									"win_loss_user.user_id." + gameBetInfo.user_id.toString()
								] = "";
								csl("check_winner updateData : ", updateData);
								...
								csl("check_winner configDetails : ", configDetails);
							}
				...
				let en = "LUCKY_CARD_12_WIN";
				if (total_win == 0) {
					en = "LUCKY_CARD_12_LOSE";
				}
