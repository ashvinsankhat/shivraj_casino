...
module.exports.check_winner = async table_id => {
	let wh1 = {};
	let gameInfos = await RouletteZeroPlayings.findOne(wh1, {}).lean();
	...
	let gameInfo = await RouletteZeroPlayings.findOneAndUpdate(wh, updateData, {
		new: true,
	}).lean();
	scl("roulette_zero_check_winner userInfos : ", gameInfo);
	...
	if (gameInfo.last_win_cards.length > 10) {
		...
		gameInfo = await RouletteZeroPlayings.findOneAndUpdate(wh, updateData, {
			new: true,
		}).lean();
				...
				for (let key = 0; key < ticket_ids.length; key++) {
					...
					if (gameBetInfo != null) {
						...
						if (total_win_amount > 0) {
							...
							let gameInfo1 = await RouletteZeroPlayings.findOneAndUpdate(
								wh,
								updateData,
								{ new: true }
							);
