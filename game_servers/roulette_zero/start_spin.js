...
module.exports.start_spin = async table_id => {
	try {
		...
		let gameInfos = await RouletteZeroPlayings.findOne(wh, {}).lean();
		...
		scl("roulette_zero_start_spin userInfos : ", gameInfo);
...
module.exports.get_random_cards = async (gameInfo, no1, i1, bet_cards) => {
	...
	if (bet_cards.indexOf(cards.toString()) != -1) {
