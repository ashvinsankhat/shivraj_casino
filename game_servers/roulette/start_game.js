...
module.exports.roulette_start_game = async() => {
    ...
    csl("roulette_start_game userInfos : ", gameInfo);
    if(gameInfo == null){
        let insertData = {
        ...
        gameInfo = await RoulettePlayings.create(insertData);
    }
    csl("roulette_start_game userInfos : ", gameInfo);
    if(gameInfo.game_state == "init_game" || gameInfo.game_state == "finish_state"){
        ...
        if(gameInfo.last_win_cards.length > 10){
            ...
            csl("roulette_start_game lastCards : ", lastCards);
            
            updateData["$set"] = {
                last_win_cards : lastCards
            }
        }
        csl("roulette_start_game wh , project 1: ", wh, updateData);
    
        gameInfo = await RoulettePlayings.findOneAndUpdate(wh, updateData, {new :true});
        csl("roulette_start_game userInfos : ", gameInfo); 
        ...
        respSendActions.FireEventToTable(gameInfo._id.toString(), "ROULETTE_INIT_GAME", response);
    
        let job_id  = "INIT:"+gameInfo._id;
        const turnExpireTime = await commonClass.AddTime(5);
        csl("roulette_start_game : ", job_id, new Date(), new Date(turnExpireTime));
    
        let table_id = gameInfo._id;
        const delayRes = await scheduleClass.setScheduler(job_id, new Date(turnExpireTime));
        csl("roulette_start_game  delayRes: ", delayRes);
        ...
        console.log("roulette_start_game Game state is different......!");
    }
                    ...
                    if(gameInfo.winner_users.indexOf(ticket_ids[key]) == -1){
                        ...
                        if(gameBetInfo != null){
                            ...
                            if(total_win_amount > 0){
                                ...
                                let gameInfo1 = await RoulettePlayings.findOneAndUpdate(wh, updateData, {new :true});
